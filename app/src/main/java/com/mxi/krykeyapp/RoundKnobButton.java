...
//		 posDegree = 360 + posDegree;
		float scaleDegrees = percentage + 180; // given the current parameters, we go from 0 to 300
		// get position percent
//		int percent = (int) (scaleDegrees / 4);
		
		setRotorPosAngle(scaleDegrees);
	}
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		...
		if (! Float.isNaN(rotDegrees)) {
			...
			if (rotDegrees < 0) posDegrees = rotDegrees;
			...
			//if (posDegrees > 210 || posDegrees < 150) {
				...
				// get a linear scale
				int scaleDegrees = (int) (rotDegrees + 180); // given the current parameters, we go from 0 to 300
				// get position percent
//				int percent = (int) (scaleDegrees / 4);
...
//					e.printStackTrace();
				...
				if (m_listener != null) m_listener.onRotate(b);
